...
#define      buttonPin         D7
//#define      Salida_analogica  D7   // (D5) 
// en D8 está el display nokia y D4 es el Led interno
...
void setup(void)
{
 ...
 // analogWrite(Salida_analogica, 0); 
...
void loop() {
  ...
  //Serial.print("estado boton:"); Serial.println(buttonState);
   if (buttonState == HIGH ) {
...
/////////////////////////////////////////////////
    ...
    // PAGE 5 - PROGRAM extractor Miel
    //------------------
    // See if Run Buttomn was pressed
    //----------------------------------------------------------------------
    if (request.indexOf("Run=") != -1) {
      ...
      Stop = false;
      ProgMielSave = true;
      Serial.println("Run program Miel");
    }
    // See if Stop Button was pressed
    //----------------------------------------------------------------------
    if (request.indexOf("Stop=") != -1) {
      ...
      Stop = true;
      ProgMielSave = true;
      Serial.println("Stop program Miel");
    }
    // See if Save Button was pressed
    //----------------------------------------------------------------------
    if (request.indexOf("SaveBtn_5=") != -1) {
      Page = 5;
      ProgMielSave = true;
    }
    // See if Clear Button was pressed
    //----------------------------------------------------------------------
    if (request.indexOf("ClearBtn_5=") != -1) {
      Page = 5;
      for (byte i = 0; i < 10; i++ ) {
        Tiempo[i] = 0;
        Velocidad[i] = 0;
    ...
    // Get program data if any button was pressed
    //----------------------------------------------------------------------
    if (ProgMielSave == true) {
      ProgMielSave = false;
      Serial.println("Leyendo valores de configuración Miel");
      
      for (j=0;j<10;j++){
         string1="t";
         string2=string1+j;
        if (request.indexOf(string2) != -1)  {
        String Tmp = request;
        int t1 = Tmp.indexOf(string2);
        Tmp.remove(0,t1+3);
        t1 = Tmp.indexOf("&");
        Tmp.remove(t1);
        Tiempo[j] = Tmp.toInt();
       ...
       for (j=0;j<10;j++){
        ...
        t1 = Tmp.indexOf("&");
        Tmp.remove(t1);
        Velocidad[j] = Tmp.toInt();
        Serial.println(Velocidad[j]);
        }
      }
       ...
       for (j=0;j<10;j++){
         ...
         if (request.indexOf(string2) != -1) Giro[j] = true;
          Serial.println(Giro[j]);
        }
...
////////////////////////////////////////////////////
    ...
    client.println("D5 Relay_dcha");
    client.println("D6 Relay_izda");
    //client.println("D7 Salida Analogica");
    ...
    if ( (TimeOk == false) and (Mode == 2) ) { 
      ...
      client.println("<br>");
      client.print(" Programa Miel : Estado:"); if (Run) {client.print("Run");} else {client.print("Stop");}client.println("<br>");
      client.print("Tiempo transcurrido:"); client.print(tiempo_transcurrido);client.println("<br>");
      client.print("Tiempo programa:"); client.print(Tiempo[numero_paso]);client.println("<br>");
      client.print("No paso ejec.:"); client.print(numero_paso);client.println("<br>");
      client.print("Sentido Giro:"); client.print(Giro[numero_paso]);
    ...
    client.print("<a href=\"Link=5\">Programa Extractor Miel</a><br>"); 
    // Draw line
    //----------------------------------------------------------------------
    client.print("</b><hr />");  
...
//      if (Error7 == true) client.print(ErrMsg); else client.print(" (dd/mm/yyyy)");
    ...
    if (Page == 5) {
      // Program number
      client.print("<font size = \"4\"><b>"); 
      client.print("Programa Extractor Miel ");
     
     
      //Run Button
      client.println("&emsp;<input type=\"submit\" name =\"Run\" value=\" Ejecutar programa \">");
      client.println("&emsp;");
      //Stop Button
      client.println("<input type=\"submit\" name =\"Stop\" value=\" Parar programa \"></font></b><br><br>");
     client.print("No de paso, Tiempo de ejecucion[s], %Velocidad[0-1023], Giro Dcha[1] o Izda[0]<br><br>");
      ...
      for(j=0;j<10;j++){
      paso=j;
      //client.print("Paso1: <input type=\"text\"<input maxlength=\"2\" size=\"2\" name=\"t1\"value =\"");
      client.print("Paso");
      client.print(paso); 
      client.print(": <input type=\"text\"<input maxlength=\"4\" size=\"4\" name=\"t");
      client.print(paso); 
      client.print("\"value =\"");
      client.print(Tiempo[paso]);    
      //client.print("\"> : <input type=\"text\"<input maxlength=\"2\" size=\"2\" name=\"t2\"value =\"");
      client.print("\"> : <input type=\"text\"<input maxlength=\"4\" size=\"4\" name=\"v");
      client.print(paso); 
      client.print("\"value =\"");
      client.print(Velocidad[paso]);    
      client.print("\">");

      //client.print("<input type=\"Checkbox\" name=\"D10\"");
      client.print("<input type=\"Checkbox\" name=\"g");
      client.print(paso);
      client.print("\"");
      if (Giro[paso]==true) client.print("checked"); else client.print("unchecked");
      client.print("> Giro Derechas<br>");
     // client.print("<br>");
...
//      client.print("> Thu<br>");
      ...
      //Button
      client.println("<br>");
      client.println("<input type=\"submit\" name =\"SaveBtn_5\" value=\"Save\">");
      client.print("&emsp;");
      client.println("<input type=\"submit\" name =\"ClearBtn_5\" value=\"Clear\">");
    }
...
//###############################################################################################
// Save Program MIEL Data
// 
//###############################################################################################
void SaveProgramMiel() {
  for (byte i = 0; i < 10; i++) {
    byte nr = i;
    byte t1 = 150 + (nr * 12);
    //On Time
    EEPROM.write(t1,Tiempo[nr]);
    EEPROM.write(t1 +  1,Velocidad[nr]);
    EEPROM.write(t1 +  2,Giro[nr]);
    EEPROM.commit();
  }
}
...
void ReadData() {
  ...
  for (byte i = 0; i < 10; i++) {
    byte nr = i;
    byte t1 = 150 + ( (i) * 12);
    Tiempo[nr]  = EEPROM.read(t1);
    Velocidad[nr]  = EEPROM.read(t1 + 1);
    Giro[nr]  = EEPROM.read(t1 + 2);
  }
...
//###############################################################################################
// Ejecuta el programa de la Miel según los pasos programados
// 
//###############################################################################################
void DoTimeCheckMiel() {
  ...
  if (Stop== true || Run!=true) {
    ...
    return;
     ...
     Output_dcha = Giro[numero_paso];
     Output_izda = !Giro[numero_paso];
   
    tiempo_transcurrido=timeNow-timeOld2;
    if( tiempo_transcurrido>Tiempo[numero_paso] ){
      numero_paso++;
      timeOld2 = timeNow;
      if(numero_paso>9){
        Stop=true;
        ...
        if (Giro[numero_paso]){
          ...
          Output_izda = false;
            ...
            Serial.println(Velocidad[numero_paso-1]);
      }
    ...
    if (Output_dcha == true){
      digitalWrite(Relay_dcha,HIGH);
      digitalWrite(Relay_izda,LOW);
     // analogWrite(Salida_analogica, Velocidad[numero_paso]); 
    }
    if (Output_izda == true){
      digitalWrite(Relay_dcha,LOW);
      digitalWrite(Relay_izda,HIGH);
    //  analogWrite(Salida_analogica, Velocidad[numero_paso]); 
    }
...
void Miel_display()
{
  ...
  display.printf( "%02u-%02u-%04u", day(), month(), year() );
  display.println(" ");
  display.printf( "%02u:%02u:%02u", hour(), minute(), second() );
  display.println(" ");
  display.print("Estado: "); 
  if (Run) {display.println(" Run"); display.print("Npaso:"); display.println(numero_paso);} else {display.println("Stop");}
  paso=false;
  }
  else{
    paso=true;
    ...
    if (Run) {
        ...
        display.print("T_paso:"); display.println(Tiempo[numero_paso]);
        display.print("Npaso:"); display.println(numero_paso);
        display.print("Giro:"); display.print(Giro[numero_paso]);
      } else {
